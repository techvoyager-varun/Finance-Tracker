...
const InsightsSection = () => {
  ...
  const insights = useMemo(() => {
    ...
    const monthlyChange = marExpenses > 0 ? (aprExpenses - marExpenses) / marExpenses * 100 : 0;
  ...
  const cards = [{
    ...
    detail: insights.largestExpense ? `${fmt(insights.largestExpense.amount)} \u2014 ${insights.largestExpense.category}` : ""
  }];
  return <div className="w-full animate-fade-in">
      ...
      <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((c, i) => <Card key={i} className="p-3 sm:p-4 card-shadow transition-all duration-300 hover:-translate-y-0.5 hover:card-shadow-hover group" style={{
