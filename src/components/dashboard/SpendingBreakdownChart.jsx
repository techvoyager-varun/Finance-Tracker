...
const COLORS = ["hsl(217, 91%, 60%)", "hsl(142, 71%, 45%)", "hsl(38, 92%, 50%)", "hsl(280, 67%, 60%)", "hsl(350, 80%, 60%)", "hsl(190, 80%, 50%)", "hsl(160, 60%, 45%)", "hsl(30, 80%, 55%)"];
const SpendingBreakdownChart = () => {
  ...
  return <Card className="p-4 sm:p-5 card-shadow animate-fade-in h-full">
      ...
      {data.length === 0 ? <div className="flex items-center justify-center h-[220px] text-sm text-muted-foreground">
          ...
          <div className="w-full h-[180px] sm:h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={v => [fmtShort(v), undefined]} contentStyle={{
              fontSize: "12px",
              border: `1px solid ${theme === "dark" ? "hsl(220, 20%, 20%)" : "hsl(220, 13%, 91%)"}`,
              backgroundColor: theme === "dark" ? "hsl(222, 47%, 11%)" : "hsl(0, 0%, 100%)"
            }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          ...
          return <div key={item.name} className="grid grid-cols-[minmax(0,1fr)_46px_84px] items-center text-xs sm:text-sm hover:bg-muted/50 transition-colors duration-150 px-1.5 py-1 gap-2">
